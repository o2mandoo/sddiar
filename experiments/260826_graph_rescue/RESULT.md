...
참고로 과거 Clova 60% assignment calibration을 다시 적용한 보조 실행은 신규
구제 `135.36초`, proxy precision `88.24%`였다. 이 보조 실행의 graph threshold는
label-independent이지만 seed가 reference-calibrated upstream에 간접 의존하므로
독립 판정으로 사용하지 않는다.
